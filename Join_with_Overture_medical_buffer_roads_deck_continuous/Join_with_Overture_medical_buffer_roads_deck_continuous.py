...
def udf(
   bbox: fused.types.TileGDF=None, overture_type="segment", clip: bool = False, 
    ...
    try:
        hos_udf = fused.load("stephen.kent.data@gmail.com/Overture_Maps_Example_hospital")
        gdf = fused.run(
            hos_udf,
            theme="base",
    ...
    import numpy as np
    gdf = gdf.to_crs(epsg=3857)
    gdf_joined = gdf_joined.to_crs(epsg=3857)
    ...
    # Ensure distance is numerical
    gdf_joined['distance'] = gdf_joined['distance'].astype(float)
    
    # Normalize the distances
    min_dist = np.min(gdf_joined['distance'])
    max_dist = np.max(gdf_joined['distance'])
    gdf_joined['normalized_distance'] = (gdf_joined['distance'] - min_dist) / (max_dist - min_dist) * 10
    
    # Convert back to EPSG:4326
    gdf_joined = gdf_joined.to_crs(epsg=4326)
    
    print(gdf_joined["normalized_distance"].describe())
    return gdf_joined