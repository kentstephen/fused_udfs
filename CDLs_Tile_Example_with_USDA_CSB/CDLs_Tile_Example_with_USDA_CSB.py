...
def udf(
    ...
    if apply_csb_mask:
        try:
            ...
            # Get the crop boundaries for the same bounds
            ...
            if crop_boundaries is not None and len(crop_boundaries) > 0:
                ...
                # Apply the mask to the CDL array
                array_int = np.where(mask, array_int, 0)
...
def create_polygon_mask(gdf, bounds, chip_len):
    ...
    # This creates a binary mask where 1 = inside polygon, 0 = outside
    mask = rasterize(
        gdf.geometry,
