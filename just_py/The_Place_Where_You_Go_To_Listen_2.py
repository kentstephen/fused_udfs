...
// Disable button until page is ready
startBtn.disabled = true;

// Enable button once page is fully loaded
...
// Calculate base color from data
...
const centerR = Math.min(255, Math.round(r));
const centerG = Math.min(255, Math.round(g));
const centerB = Math.min(255, Math.round(b));

// Create Turrell-style radial gradient with glowing edges
const centerX = w / 2;
const centerY = h / 2;
...
gradient.addColorStop(0, `rgba(${{centerR + 40}}, ${{centerG + 40}}, ${{centerB + 40}}, 1)`);
gradient.addColorStop(0.3, `rgb(${{centerR}}, ${{centerG}}, ${{centerB}})`);
// Edge glow (more saturated at edges)
gradient.addColorStop(0.7, `rgb(${{Math.min(255, centerR * 1.2)}}, ${{Math.min(255, centerG * 1.2)}}, ${{Math.min(255, centerB * 1.3)}})`);
gradient.addColorStop(1, `rgb(${{Math.min(255, centerR * 1.4)}}, ${{Math.min(255, centerG * 1.4)}}, ${{Math.min(255, centerB * 1.6)}})`);
...
// Combine all seismic into one frequency/volume
const totalSeismic = d.seismic_local + d.seismic_regional * 0.5 + d.seismic_global * 0.3;
const seismicFreq = 60 + totalSeismic * 80;
const seismicVol = 0.1 + totalSeismic * 0.15;
mainOsc.frequency.linearRampToValueAtTime(seismicFreq, t + rampTime);
mainGain.gain.linearRampToValueAtTime(seismicVol, t + rampTime);

// Aurora
const auroraVol = d.aurora_intensity * 0.1;
auroraGain.gain.linearRampToValueAtTime(auroraVol, t + rampTime);

// Solar noise with filter
const filterFreq = 500 + d.solar_brightness * 1500;
window.solarFilter.frequency.linearRampToValueAtTime(filterFreq, t + rampTime);
const solarVol = d.solar_brightness * 0.03;
solarGain.gain.linearRampToValueAtTime(solarVol, t + rampTime);
}}
</script>
</body>
</html>"""
