...
def udf(
    ...
    # cali_clipped = gdf_cali.clip(tile)

    # Explicit empty check
    # if cali_clipped is None or len(cali_clipped) == 0 or cali_clipped.is_empty.all():
    #     return None

    xmin, ymin, xmax, ymax = bounds
    zoom = common.estimate_zoom(bounds)

    pyramid_level = max(0, min(zoom - 1 + 5, 12))
    ...
    # Create xarray and clip to California using rioxarray
    da = xr.DataArray(
        arr,
        dims=['y', 'x'],
        coords={
    ...
    # da = da.rio.clip(cali_clipped.geometry, cali_clipped.crs, drop=False)

    arr = da.values

    # Data mask (valid data after clipping)
    ...
    alpha = data_mask.astype(np.uint8) * 150
