...
def udf(bounds: fused.types.Bounds= [-126.6,20.9,-64.9,50.4], 
    ...
    if png:
        ...
        # df_final = duckdb.sql("select df.hex, round(df.metric::double,4) as burn_prob, df_census.* exclude(df_census.hex) from df join df_census on df.hex = df_census.hex where df_census.POP20 >=10").df()
        df_final = duckdb.sql("""                                                                                                                  
          SELECT                                                                                                                                 
...
def load_from_url(bounds):
    ...
    arr = fused.run("fsh_1ccXDvLBAFDUtQ9JP1ZBBQ", bounds=bounds)
