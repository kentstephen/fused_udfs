...
def udf(bounds: fused.types.Bounds=None,
    ...
    df_elev, df_flow = common.run_pool(
        ...
        ["fsh_7EALhZzZ4nKSfhtXGrGjDQ", "fsh_330hwPPsFSELyDF604P7Yj"],
        max_workers=2
    ) 
    ...
    qr = """select 
               ...
               e.metric as elev,
               --f.metric as flow,
                e.metric + (LOG10(f.metric + 1) * 375) as e_f
            --   (e.metric - 1900) * 60 + SQRT(f.metric) * 40 as e_f -- kinda weird, large values
              -- (e.metric - 250) * (1 + LOG10(GREATEST(f.metric + 1, 1)) * 0.194) as e_f
             --   (e.metric - 1900) * (1 + LOG10(f.metric + 1) * 05) as e_f,
