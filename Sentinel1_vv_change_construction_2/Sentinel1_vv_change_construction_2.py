...
# Filter for ascending only (or descending - pick one)
    ...
    # Assuming your dataset has a time coordinate
# First get the diff as you did
    ds['year'] = ds.time.dt.year  # This adds year info right at the start
...
#     arr = np.ma.masked_array(arr, mask=geom_mask)
    ...
    # df_vv['year'] = ds['year']
    print("Length of df_vv:", len(df_vv))
    print("Length of years:", len(ds.time.dt.year))
    # print("Raw data stats:")
...
SELECT 
    h3_h3_to_string(h3_latlng_to_cell(lat::double, lng::double, {h3_size}::integer)) AS hex,
