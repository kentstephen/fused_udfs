...
def udf(bounds: fused.types.Bounds=None,
    ...
    import duckdb
    udf_elev ="fsh_la0osbNdypUY658q7K3bt"
    udf_flow = "fsh_3FSpbctQi4lgf06LUYTiXv"
    common = fused.load("https://github.com/fusedio/udfs/tree/b7637ee/public/common/")
    df_elev, df_flow = common.run_pool(
        lambda token: fused.run(token, res=res, bounds=bounds, bucket_path=bucket_path),
        [udf_elev, udf_flow],
        max_workers=2
    )
    ...
    qr = """select 
          ...
          e.metric as elev,
            --  f.metric as flow,
         (e.metric - 350) - (1 + LOG10(least(greatest(f.metric)) + 1) * 0.9) as e_f,
     --(e.metric -370) + log(least(greatest(f.metric, 0), 100) + 1) as e_f
