...
def udf(
    ...
    provider="AWS",
    time_of_interest="2023-06-25/2023-08-10",
    res:int=11,
    res_factor:int=4
):  
    """
    ...
    if len(items) < 1:
        ...
        # MNDWI (G−S1)/(G+S1)
        mndwi = (ds[green_band] - ds[swir1_band]) / (ds[green_band] + ds[swir1_band])
        print(mndwi.shape)
        arr = mndwi.max(dim="time")
        print(f"Max: {arr.max()}")
        print(f"Min: {arr.min()}")
        arr = arr.where(arr > 0.13)
        print(arr)
        ...
        rgb_image = common_utils.visualize(
            ...
            min=0,
            max=1,
            colormap=palettable.lightbartlein.sequential.Blues10_10
        )
        return rgb_image
