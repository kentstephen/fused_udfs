...
def udf(bbox: fused.types.TileGDF = None, release: str = "2024-09-18-0", theme: str = None, overture_type: str = None, use_columns: list = None, num_parts: int = None, min_zoom: int = 0, polygon: str = None, point_convert: str = None,
       ...
       min_count: int=25,
        limit: int=10_000):
    from utils import get_overture, get_con, add_rgb
    import shapely 
    ...
    @fused.cache
    def get_cabs():
        cab_udf = fused.load('https://github.com/fusedio/udfs/tree/e3a20a4/public/DuckDB_H3_Example/')
        gdf = fused.run(udf=cab_udf, resolution=resolution, min_count=min_count)
        return gdf
    ...
    gdf_joined = gdf_overture.sjoin(cab_gdf)
