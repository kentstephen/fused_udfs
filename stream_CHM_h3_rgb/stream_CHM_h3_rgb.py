...
def udf(
    ...
    import palettable
    import duckdb
    from utils import aggregate_df_hex, df_to_hex, add_rgb
    ...
    import numpy as np
    df['viz_metric'] = np.where(df['metric'] == 0, 0, np.sqrt(df['metric']) * 30)
    ...
    df.*, 
    --df_dem.metric + 
    df.viz_metric as total_elevation from df 
    inner join df_dem on df.hex=df_dem.hex""").df()
    df = add_rgb(df=df, value_column='metric', n_quantiles=10)
    # df = df[df['metric']>0]
    print(df)
    return df
    # print(f"{arr.max()=}")
