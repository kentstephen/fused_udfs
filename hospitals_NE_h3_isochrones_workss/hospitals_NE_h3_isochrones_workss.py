@fused.udf
def udf(bbox: fused.types.TileGDF=None, n: int=10, path:str="s3://fused-users/stephenkentdata/hospitals_NEW_ENG_r_8_iso2.parquet"):
    ...
    @fused.cache
    def run_query(df_iso, df_kontur_us, df_kontur_ca, mask_wkt):
        con = fused.utils.common.duckdb_connect()
        query = f"""
        select
            h3_h3_to_string(i.hex) as hex,
            i.cnt as cnt,
    ...
    df = run_query(df_iso, df_kontur_us, df_kontur_ca, mask_wkt)
    # print(df)
    # df = add_rgb(df, 'cnt')
    return df
