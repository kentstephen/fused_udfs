...
def udf(bounds: fused.types.Bounds,
        res: int =10,
        time_of_interest="2024-06-01/2024-08-24",
        scale:float=0.099,
       ):
    import geopandas as gpd
    import pandas as pd
    from utils import get_over, run_query, get_census
    ...
    #     return
    df_roads = get_census(tile)
    print(df_roads)
    # df_roads = get_over(tile, overture_type="land_use")
    # df_roads = df_roads[df_roads['subtype'].isin(['protected', 'park'])]
    # df_roads = df_roads[]
    ...
    df_sentinel = fused.run("fsh_3IksBHSe6v4V84LHoussXy", bounds=tile, time_of_interest=time_of_interest, res=res)
