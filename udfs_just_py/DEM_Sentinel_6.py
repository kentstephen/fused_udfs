...
def udf(bounds: fused.types.Bounds=None,
        time_of_interest="2024-03-25/2024-05-15", 
        chip_len:int=768, 
        scale:float=0.08766807958458571,
    ...
    arr = udf_sentinel.utils.get_arr(tile, time_of_interest=time_of_interest, output_shape=(chip_len, chip_len), max_items=1000)
    if arr is None:
        return
    ...
    # df_dem = fused.run("fsh_7SXAfMlrkS9twhxZiO2cup", bounds=bounds, res=h3_size)
    # df_dem = fused.run("fsh_5FZlHaVg8BqL2Eh2KTWevp", bounds=bounds, h3_size=h3_size) #NH USGS newfound
    
    if df_dem is None or df_dem.empty:
        return
    df = duckdb.sql("select df.*, df_dem.metric from df inner join df_dem on df.hex = df_dem.hex where df_dem.metric < 200").df()
