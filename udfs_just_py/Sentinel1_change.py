...
def udf(
    ...
    provider="AWS",
    time_of_interest="2020-01-01/2024-12-31",
    h3_size:int= 10
):  
    """
...
# Filter for ascending only (or descending - pick one)
    ...
    # 1. Calculate speckle-reduced VV for first and last time steps
# Lee filter or multi-temporal averaging could be applied here if needed
    # Multi-image approach - look at the full time series
    # Calculate temporal standard deviation or variance
    # Focus specifically on large positive changes which would indicate construction
    first_vv = ds.vv.isel(time=0)
    last_vv = ds.vv.isel(time=-1)
    total_change = last_vv - first_vv
    
    # Only keep positive changes (new structures)
    positive_change = xr.where(total_change > 0, total_change, 0)
    
    # Get values array
    arr = positive_change.values
