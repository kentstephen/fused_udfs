...
def udf(bounds: fused.types.Bounds=None,
        ...
        chip_len:int=768, 
        scale:float=0.0766807958458571,
    ...
    import numpy as np
    import duckdb
    from utils import tile_to_df, df_to_hex, get_arr
    ...
    utils = fused.load("https://github.com/fusedio/udfs/tree/ee9bec5/public/common/")
    # create a geom mask
    ...
    if df_dem is None or df_dem.empty:
        return
    df = duckdb.sql("select df.*, df_dem.metric from df inner join df_dem on df.hex = df_dem.hex -- where df_dem.metric < 500").df()
