...
def udf(bounds: fused.types.Bounds=None,
        time_of_interest="2021-05-01/2024-08-15", 
        chip_len:int=768, 
        scale:float=0.06571,
        h3_size: int = 11):    
    ...
    df_dem = fused.run("fsh_5FZlHaVg8BqL2Eh2KTWevp", bounds=bounds, h3_size=h3_size)
    
    if df_dem is None or df_dem.empty:
        return
    df = duckdb.sql("select df.*, df_dem.metric, df_dem.elev_scale from df left join df_dem on df.hex = df_dem.hex").df()
