...
def udf(bounds: fused.types.Bounds,
    ...
    walking_types = ['0855d102bfffffff045ea9d978a0deee','0845d103ffffffff042ebed395f2031f','0875d10281ffffff047ea40f4f70b46d', '0855d102bfffffff043fad957cd0986b']
    gdf = gdf[gdf['id'].isin(walking_types)]
    # print(gdf['class'])
    # return gdf
    ...
    if gdf is not None and not gdf.empty:
       gdf['geometry'] = gdf.to_crs(gdf.estimate_utm_crs()).buffer(45).to_crs('EPSG:4326')
    else:
        return
    # Convert to WKT
...
to_grid as (
  select
    unnest(h3_grid_disk(ae.hex, 7)) as disk_hex,
    metric
  from add_elevation ae
)
select 
  a.hex,
 -- stddev(t.metric) as stddev,
  a.metric,
  --array_agg(a.class) as class,
 -- array_agg(a.id) as id
