...
def arr_to_cmap(data, color_palette, norm_range=(0, 255)):
    # Create a colormap from the provided hex color palette
    cmap = LinearSegmentedColormap.from_list("custom_cmap", color_palette, N=256)

    # Normalize data
    data = data.astype(float)
    if norm_range:
        norm_data = np.clip((data - norm_range[0]) / (norm_range[1] - norm_range[0]), 0, 1)
    else:
        norm_data = np.clip((data - np.nanmin(data)) / (np.nanmax(data) - np.nanmin(data)), 0, 1)
    
    # Apply colormap
    norm_data255 = (norm_data * 255).astype(np.uint8)
    mapped_colors = cmap(norm_data255)  # This will give us RGBA
    mapped_colors = (mapped_colors[..., :3] * 255).astype(np.uint8)  # Convert to 8-bit RGB

    # Reshape and transpose to fit the expected format [channel, height, width]
    return np.transpose(mapped_colors, (2, 0, 1))
...
def arr_to_plasma(data, min_max=(0,255)): 
    ...
    if min_max:
        ...
        print('min_max:', np.nanmin(data),np.nanmax(data))
