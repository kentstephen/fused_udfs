...
def udf(
    ...
    lat_slice = lat[lat_idx.min():lat_idx.max()+1][::-1]

    # Reproject to 3857
    da = xr.DataArray(arr, dims=['y', 'x'], coords={'y': lat_slice, 'x': lon_slice})
    da = da.rio.write_crs("EPSG:4326")
    da = da.rio.reproject("EPSG:3857", shape=(chip_len, chip_len))
    arr = da.values
    ...
    alpha = (mask > 0.5).astype(np.uint8) * 155
