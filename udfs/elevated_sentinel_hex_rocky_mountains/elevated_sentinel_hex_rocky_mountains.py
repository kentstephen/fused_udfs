...
def udf(bounds: fused.types.Bounds, 
        time_of_interest="2020-05-01/2020-06-20", 
        chip_len:int=256, 
        scale:float=0.07,
       h3_size: int = 12):
    ...
    df = duckdb.sql("install h3 from community; load h3; select df.*, df_dem.metric from df left join df_dem on df.hex = h3_h3_to_string(df_dem.hex)").df()
    df['metric'] = df["metric"] - 2150
    # df = df[df["metric"]>70]
    print(df)
    return df