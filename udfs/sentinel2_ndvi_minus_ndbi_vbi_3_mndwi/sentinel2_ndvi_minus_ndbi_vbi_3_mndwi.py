...
def udf(
    ...
    if len(items) < 1:
        ...
        mndwi = (ds[green_band] - ds[swir_band]) / (ds[green_band] + ds[swir_band])
        # mndwi = mndwi.where(mndwi > 0)  # Only positive MNDWI values
        ndvi = (ds[nir_band] - ds[red_band]) / (ds[nir_band] + ds[red_band])
        # vbi = xr.where(mndwi > 0, 
        #        1.5,
        #        ndvi - ndbi + mndwi)
        vbi = ndvi - ndbi + mndwi
        # water_areas = mndwi > 0  # Much lower for rivers
        # vbi_with_water = vbi.where(~water_areas, 1.5)  # Force water to high value
        print(vbi.shape)
        ...
        arr.values,
        min=-0.6,  # Slightly below mean (cooler areas)
        max=1.6,  # Slightly above mean (warmer areas)
        colormap=palettable.cartocolors.diverging.Fall_7_r,
    )
    #     # return rgb_image
