...
def udf(bounds: fused.types.Bounds= [-126.6,20.9,-64.9,50.4], 
    ...
    if png:
        ...
        if df is None or len(df)<0:
            return None
        df_census = fused.run("fsh_1FIkMRXsTjZblzGOrsf09z",bounds=bounds)
        full_outer_join = """  
            SELECT                                                                                                                                     
              COALESCE(df.hex, df_census.hex) as hex,                                                                                                
              COALESCE(round(df.metric::double, 4), 0) as burn_prob,                                                                                 
              COALESCE(df_census.POP20, 0) as pop                                                                                                    
          FROM df                                                                                                                                    
          FULL OUTER JOIN df_census ON df.hex = df_census.hex                                                                                        
          WHERE df_census.POP20 >= 1 OR df.metric > 0           
          """
        df_final = duckdb.sql(full_outer_join).df()
        return df_final
        # df_final = duckdb.sql("select  coalesce(df.hex, df_census.hex) as hex,  round(df.metric::double,4) as burn_prob, df_census.POP20 as pop from df full outer join df_census on df.hex = df_census.hex where df_census.POP20 >=1").df()
        # df_final = duckdb.sql().df()
      #   ratio_qr = """                                                                                                                  
      #     SELECT                                                                                                                                 
      #         coalesce(df.hex, df_census.hex) as hex,                                                                                                                            
      #         round(df.metric::double, 4) as burn_prob,                                                                                          
      #         df_census.* EXCLUDE(hex),                                                                                                          
      #         -- Population at risk                                                                                                              
      #         round(df.metric::double * df_census.POP20, 0) as pop_at_risk,                                                                      
      #         -- Normalized risk score (0-1)                                                                                                     
      #         round((df.metric::double * 1000.0) * (df_census.POP20 / 1000.0), 4) as risk_score,                                                            
      #         -- Housing at risk                                                                                                                 
      #         round(df.metric::double * df_census.HOUSING20, 0) as housing_at_risk,
      #           round(                                                                                                                                     
      #             0.5 * (COALESCE(df.metric::double, 0) / 0.15) +  -- burn normalized (assuming 0.15 max)                                                
      #             0.5 * (COALESCE(df_census.POP20, 0) / 10000.0),  -- pop normalized (adjust max)                                                        
      #             4                                                                                                                                      
      #         ) as combined_score      
      #     FROM df                                                                                                                                
      #     full outer JOIN df_census ON df.hex = df_census.hex                                                                                               
      #     --WHERE df_census.POP20 >= 1                                                                                                            
      #     ORDER BY pop_at_risk DESC                                                                                                              
      # """
...
def load_from_url(bounds):
    ...
    arr = fused.run("fsh_59gkcLyhLlZbeXXaNnOMiM", bounds=bounds)
