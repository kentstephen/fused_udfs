...
def run_query(df_sentinel, res, bounds, tile, overture_type):
    import geopandas as gpd
    import shapely
    xmin, ymin, xmax, ymax = bounds
    utils = fused.load("https://github.com/fusedio/udfs/tree/ee9bec5/public/common/").utils
    # Connect to DuckDB
    df_overture = get_over(tile=tile, overture_type=overture_type)
    df_fields = table_to_tile(bounds)
    print(df_fields)
    con = utils.duckdb_connect()
    query=f"""    with nyc_boundary as (
    
select
 unnest(h3_polygon_wkt_to_cells_experimental(geometry, {res}, 'center')) as hex,
df_fields.*
 from df_fields
), overture_buildings as (
select
 unnest(h3_polygon_wkt_to_cells_experimental(geometry, {res}, 'center')) as hex,
geometry, height, id
 from df_overture
...
FROM 
  overture_buildings o

inner JOIN 
  nyc_boundary r ON o.hex = r.hex inner join df_sentinel s on s.hex = o.hex
