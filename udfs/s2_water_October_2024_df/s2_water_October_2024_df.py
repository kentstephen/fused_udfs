...
def udf(bbox: fused.types.TileGDF=None, 
        resolution: int = 13,
       time_of_interest="2024-09-15/2024-11-01"):
    ...
    water_mask = (ds.image.sel(band=2) < 50).values.astype(float) #NIR
    ...
    df_pop = arr_to_latlng(water_mask, bounds)
    print(df_pop)
    print(df_pop)
    def df_to_hex(df, resolution, latlng_cols=("lat", "lng")):
        ...
        qr = f"""
                SELECT h3_latlng_to_cell(lat, lng, {resolution}) AS hex, sum(data) as data
                FROM df
                group by 1
              --  order by 1
            """
    ...
    df = df_to_hex(df_pop, resolution)
    df = df[df["data"] > 0]
