...
def udf(
    ...
    from utils import aggregate_df_hex, get_over,run_query, visualize
    ...
    if len(items) < 1:
        ...
        # df_dem = fused.run("fsh_3ailz7Y3EAoBnz7G4pPYPJ", bounds=bounds, res=res, tree_scale=tree_scale) # meta trees
        ...
        # df = duckdb.sql(" select df.hex, df.metric as ndvi, df_dem.total_elevation, df_dem.canopy_height from df left join df_dem on df.hex = df_dem.hex").df()
        ...
        arr.values,
        min=0,  # Slightly below mean (cooler areas)
        max=1,  # Slightly above mean (warmer areas)
        colormap=palettable.cartocolors.sequential.BluGrn_7_r,
    )
