def df_to_hex(df, res, bounds, latlng_cols=("lat", "lng")):
    ...
    # return con.query(qr).df()
def aggregate_df_hex(df, res,bounds, latlng_cols=("lat", "lng"), stats_type="mean"):
    import pandas as pd
    import numpy as np
    
    # Convert to hexagons
    df = df_to_hex(df, res=res, bounds=bounds, latlng_cols=latlng_cols)
