...
def process_hex_data(df, colormap):
    """Efficiently process hex data with vectorized operations"""
    import numpy as np
    import pandas as pd
    
    # Find most frequent value per hex
    def get_most_frequent(agg_data):
        unique_vals, counts = np.unique(agg_data, return_counts=True)
        max_idx = np.argmax(counts)
        return unique_vals[max_idx], counts[max_idx]
    
    # Apply vectorized operation
    freq_data = df['agg_data'].apply(get_most_frequent)
    df['most_freq'] = [x[0] for x in freq_data]
    df['n_pixel'] = [x[1] for x in freq_data]
    
    # Filter out zero values
    df = df[df['most_freq'] > 0].copy()
    
    if len(df) == 0:
        return df
    
    # Vectorized color mapping
    df[['r', 'g', 'b', 'a']] = pd.DataFrame(
        df['most_freq'].map(colormap).tolist(), 
        index=df.index
    )
    
    # Map crop types
    df['crop_type'] = df['most_freq'].map(int_to_crop)
    df['color'] = df['most_freq'].apply(
        lambda x: nlcd_example_utils.rgb_to_hex(colormap[x]) if x in colormap else "NaN"
    )
    
    # Return only needed columns
    return df[['hex', 'crop_type', 'color', 'r', 'g', 'b', 'a', 'most_freq', 'n_pixel']].copy()
