...
def udf(
    ...
    df_road = fused.run("fsh_6xQ2FETnectylQh8ihB544", bounds=bounds, res=res)
    if df_road is not None and not df_road.empty:
        road_hex_values = df_road['hex'].unique()
        
        # Create a mask that identifies all rows in df where the hex matches any value in road_hex_values
        mask = df['hex'].isin(road_hex_values)
        
        # Set matching rows' aggregation bands to None/null
        df.loc[mask, 'metric'] = None
