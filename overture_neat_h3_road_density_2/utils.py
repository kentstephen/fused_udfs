def get_overture(bounds):
        ...
        df = pd.DataFrame(gdf)
        return df
def run_query(resolution, df_segment, df_dem):
        ...
        query = f"""
            WITH geometry_cte AS (
                ...
                SELECT
                    ...
                    sum(d.metric) as elevation
                FROM h3_cells h join df_dem d
               on h.cell_id = d.hex
                GROUP BY cell_id
                )
                SELECT cell_id, total_value, elevation FROM aggregated_cells;
                """
        df = con.sql(query).df()
        return df
