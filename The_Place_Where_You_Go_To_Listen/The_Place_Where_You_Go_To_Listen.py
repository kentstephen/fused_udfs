...
def udf(location: str = "New York", format: str = "page"):
    ...
    def make_wave(data, dur=30, sr=44100):
        """Generate a WAV file with harmonic C major tones driven by data."""
        ...
        # C MAJOR HARMONIC STRUCTURE - DEEP BASS with subtle upper harmonics
        # Local seismic - Sub bass C0 (16.35 Hz) - extremely deep, felt more than heard
        local_bass = tone(45, data["seismic_local"] * 0.20)
        
        # Regional seismic - Deep bass G1 (49 Hz) 
        regional_mid = tone(100, data["seismic_regional"] * 0.15)
        
        # Global seismic - Bass E2 (82.41 Hz)
        global_high = tone(82.41, data["seismic_global"] * 0.12)
        
        # Aurora - Very low C-E-G chord (65-98 Hz) with minimal upper brightness
        lfo_depth = data["aurora_intensity"] * 3
        lfo = np.sin(1.7 * np.pi * 0.2 * t) * lfo_depth  # Slower LFO too
        aurora_c = np.sin(2 * np.pi * (65.41 + lfo) * t) * data["aurora_intensity"] * 0.15
        aurora_e = np.sin(2 * np.pi * (82.41 + lfo * 1.2) * t) * data["aurora_intensity"] * 0.12
        aurora_g = np.sin(2 * np.pi * (98.00 + lfo * 0.8) * t) * data["aurora_intensity"] * 0.10
        
        # Optional: tiny bit of upper harmonic for brightness when aurora is strong
        aurora_bright = np.sin(2 * np.pi * 196 * t) * data["aurora_intensity"] * 0.03
        
        # Solar - Very subtle noise texture
        solar_noise = np.random.uniform(-1, 1, int(sr * dur)) * data["solar_brightness"] * 0.003
        
        # Mix all components - the mix changes based on real-time data
        mix = local_bass + regional_mid + global_high + aurora_c + aurora_e + aurora_g + aurora_bright + solar_noise
        
        # Normalize to int16 PCM
        if np.max(np.abs(mix)) > 0:
            mix = np.int16(mix / np.max(np.abs(mix)) * 32767 * 0.7)
        else:
            mix = np.int16(mix)
    ...
    hour_angle = (local_hour - 12) * 15  # Only calculate this ONCE
    day_of_year = now.timetuple().tm_yday
    data["solar_position"] = local_hour / 24.0
...
locationSelect.addEventListener('change', function() {{
    // Disable both controls while loading
