...
def udf(
    ...
    # df = df[df["metric"] > 150]
    df_ches = fused.run('fsh_310Dm67rvqJdMxHiFwFINH', bounds=bounds, res=res)
    if df_ches is not None:
        df = duckdb.sql("""
        -- Start with bathymetry as the base, then fill gaps with DEM
    SELECT 
        b.hex,
        b.metric
    FROM df_ches b
    ...
    SELECT 
        ...
        d.metric
    FROM df d
    WHERE d.hex NOT IN (SELECT hex FROM df_ches);
        """).df()
    else: return df
    print(df)
    return df
